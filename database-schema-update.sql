-- Database Schema Update for Community App
-- This script safely adds new tables and functions without conflicts
...
-- Community Polls table - only create if not exists
...
-- Poll Options table - only create if not exists
...
-- Poll Votes table - only create if not exists
...
-- Create functions for updating counts (only if they don't exist)
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        AND policyname = 'Users can read public posts'
    ) THEN
    ...
    IF NOT EXISTS (
        ...
        AND policyname = 'Users can read discussions'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'discussions' 
        AND policyname = 'Users can create discussions'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'discussion_replies' 
        AND policyname = 'Users can read replies'
    ) THEN
    ...
    IF NOT EXISTS (
        ...
        AND policyname = 'Users can read conversations'
    ) THEN
    ...
    IF NOT EXISTS (
        ...
        AND policyname = 'Users can read messages'
    ) THEN
